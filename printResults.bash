...
# performance trial for each combination of number of nodes, number of threads,
# and number of clusters performance tested and prints the average run times for
# each configuration.  The average dissemination, average modeling, and average
# total time (dissemination plus modeling time) are printed.  The dissemination
# time is the time it takes to construct the H2O distributed data structure
# before calling the H2O distributed clustering routine.  The modeling time is
# the time taken to solve the clustering problem once the distributed data
# structure has been built, and the total time is the sum of the dissemination
# and modeling times.  If a log file exists, but does not contain any timing
# information, then a warning is printed.  If the number of trials is not as
# expected, then that is an indication that a trial failed to execute.
#
